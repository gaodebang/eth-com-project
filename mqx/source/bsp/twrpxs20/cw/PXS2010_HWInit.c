...
/*
 *
 * FILE : PXS2010_HWInit.c
 *
 * DESCRIPTION:
 *  This file contains all PXS2010 derivative needed initializations, 
 *  and all initializations for the PXS2010 boards which are supported.
...
#include "PXS2010_cw.h"       /* MPC55xx platform development header            */
#include "PXS2010_HWInit.h"
...
/*******************************************************/
/* PXS2010 derivative specific hardware initialization */
/*******************************************************/
...
/* Symbol L2SRAM_LOCATION is defined in the application linker command file (.lcf) 
   It is defined to the start of the L2SRAM of the PXS2010. 
*/
...
__asm void INIT_Derivative(void) 
{
    ...
    /* PXS2010 L2SRAM initialization code */
...
/*******************************************************/
/* PXS2010 boards specific hardware initializations    */
/*******************************************************/
...
/* Recover from SAFE mode and switch to DRUN */
void SafemodeRecover(void)
{
    int i;
    ...
    /* Clear flag in RGM */
    RGM.FES.B.F_FCCU_SAFE = 1;

    /* Now it is possible to switch to  DRUN */
    ME.MCTL.R = 0x30005AF0;  /* Write target mode | key */
    ME.MCTL.R = 0x3000A50F;  /* Write target mode | inverted key */
...
/* Initialize clocks and switch to RUN0 mode */
void ClockInit(void)
{
    CGM.CMU_0_CSR.R = 0x00000006;   /* Avoid CMU reset when fXOSC<fIRC */

    /* AUX clock 0 selection */
    CGM.AC0SC.R = 0x05000000;       /* PLL_1 output as Motor control clock */
    CGM.AC0DC.R = 0x80000000;       /* Divided by 1, prescaler enabled */

    /* AUX clock 1 selection */
    CGM.AC1SC.R = 0x04000000;       /* PLL_0 output for FlexRay clock */
    CGM.AC1DC.R = 0x81000000;       /* Divided by 2, prescaler enabled */

    /* AUX clock 2 selection */
    CGM.AC2SC.R = 0x04000000;       /* PLL_0 output for FlexCAN clock */
    CGM.AC2DC.R = 0x81000000;       /* Divided by 2, prescaler enabled */

    /* AUX clock 3 selection */
    CGM.AC3SC.R = 0x01000000;       /* Clock souce for FMPLL_0 to XOSC */

    /* AUX clock 4 selection */
    CGM.AC4SC.R = 0x01000000;       /* Clock souce for FMPLL_0 to XOSC */

    /* Configure PLL_0, PLL_1 */
    //CGM.FMPLL[0].CR.R = 0x01280000; /* 80MHz PLL_0 @ 8MHz XOSC */
    //CGM.FMPLL[1].CR.R = 0x013C0000; /* 120MHz PLL_1 @ 8MHz XOSC */
    CGM.FMPLL[0].CR.R = 0x1D400000; /* 80MHz PLL_0 @ 40MHz XOSC */
    CGM.FMPLL[1].CR.R = 0x113C0000; /* 120MHz PLL_1 @ 40MHz XOSC */

    ME.RUN[0].R = 0x0001F00F4;      /* PLL0 output as sysclk, PLL0/1 ON, XOSC ON, 16MHz_IRC ON, FLASH in normal mode */
    ME.RUNPC[0].R = 0x00000010;     /* Peripherals run in RUN0 only */

    /* Initiate DRUN -> RUN0 mode transition */
    ME.MCTL.R = 0x40005AF0;         /* Write target mode | key */
    ME.MCTL.R = 0x4000A50F;         /* Write target mode | inverted key */

    /* Wait for DRUN -> RUN0 mode transition */
    while(ME.GS.B.S_MTRANS == 1) {}
...
nofralloc
    ...
    /*  This initializes the PXS2010 external bus
        Set up the pins
...
__asm void __initEBIChipSelects(void)
{
#if INIT_USED_BOARD==PXS2010DEMO_AXM_0321
