...
void ClockInit(void)
{
    ...
    /*  Set ERFD to divide by 10 at first to avoid cpu overclocking -
     ** should not occur since system is not clocked from PLL 
     ** Set PLL to use crystal reference (40MHz),
     ** EMFD=2C to multiply by 60,
	 ** and EPREDIV=4 to divide by 5
     ** Result:    fvco = 40MHz * 60 / 5 = 480MHz (192-600MHz)
     **            fout = 40MHz * 60 / ( 5 * 10) = 48MHz
     */
    FMPLL.ESYNCR2.R=0x00000009;
    FMPLL.ESYNCR1.R=0x7004002C;         
                                    
    /* Wait for PLL lock (<400us) */
    while(FMPLL.SYNSR.B.LOCK != 1)  ;

    /* ERFD now divides only by 6 so
	 * fout= fvco / (ERFD+1)
     * fout= 40MHz * 60 / (5 * 6) = 80MHz
     */
    ...
    /* System clock is divided by 1 = 80MHz
     * (grater or equal to all periph. clocks)
     */
    SIU.SYSCLK.B.SYSCLKDIV = 0b000;  
                                    
    /* eSCI, I2C clock divided by 2 = 40MHz */
    SIU.SYSCLK.B.LPCLKDIV0 = 0b01;  
    /* FlexCAN, SPI clock divided by 1 = 80MHz */
    SIU.SYSCLK.B.LPCLKDIV1 = 0b00;  
    /* ADC, CTU clock divided by 2 = 40MHz */
    SIU.SYSCLK.B.LPCLKDIV2 = 0b01;  
    /* eMIOS clock divided by 2 = 40MHz */
    SIU.SYSCLK.B.LPCLKDIV3 = 0b01;  
    /* IOP, FEC, MLB, DIM SysClk/2 always
     * FlexRay SysClk and 4-40MHz Extal
     * FlexCAN (also Extal source)
     */

    /*----- SIU - CLKOUT for debugging pupose -----*/
    /* Set pin as CLKOUT PK9 */
    SIU.PCR[153].B.PA=0b01;
    SIU.PCR[153].B.OBE=1;  
    SIU.PCR[153].B.ODE=0;
    SIU.PCR[153].B.SRC=0b11;        
    /* Enable CLKOUT */
    SIU.ECCR.B.ECEN=1;              
    /* CLKOUT/4 = 20MHz (1 to 8) */
    SIU.ECCR.B.ECDF=0b10;           
