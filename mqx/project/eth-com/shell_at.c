...
void shell_server_task(uint_32 initial_data)
{
    ...
    while(TRUE)
    {
        ...
        else if(count > 1)
        {
            ...
            {
                if(data_buffer[count]==0x0D)
                {
                    if(count == 0)
                    { 
                        data_buffer[count+1] = 0x0A;
                        data_buffer[count+2] = 0x0A;
                        write(SCI0, data_buffer, count+2);
                        fflush(SCI0);
                    }
                    else
                    {
                ...
                else if((data_buffer[count]==0x0A)&&(data_buffer[count-1]==0x0D))
                {
                    if(count == 2)
                    {   
                        data_buffer[count+1] = 0x0A;                 
                        write(SCI0, data_buffer, count+1);
                        fflush(SCI0);
                    }
                    else
                    {
                ...
                else if((data_buffer[count]==0x0D)&&(data_buffer[count-1]==0x0A))
                {
                    data_buffer[count+1] = 0x0A;                 
                    write(SCI0, data_buffer, count+1);
                    fflush(SCI0);
